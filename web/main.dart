...
//  querySelector('#output').text = 'Your Dart app is running.';
  ...
  TextAreaElement? input = document.getElementById('input_text') as TextAreaElement?;
  input?.text = startingJson;

  querySelector('#convert')?.onClick.forEach((MouseEvent event) async {
    try {
      TextInputElement? rootClassNameElement =
          document.getElementById('root_class_name') as TextInputElement?;
      String? rootClassName = rootClassNameElement?.value;

      TextAreaElement? elementById = document.getElementById('input_text') as TextAreaElement?;
      String? json = elementById?.value;
      print('json is $json');
      Parser parser = Parser();
      if (json != null && rootClassName != null) {
